...
The local patch is on `TradeResponse.fee_rate_bps` and the nested
`MakerOrder.fee_rate_bps`: it deserializes an empty-string response as zero.
The live CLOB has returned that malformed value for optional fee metadata. It
is not used for position, receipt, or reconciliation accounting in this
engine.
